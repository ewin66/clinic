...
namespace Clinic
{
    public partial class Services : Form
    {
        ...
        private void buttonServicesOK_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxServices.Text.Contains(','))
            {
                MessageBox.Show("Tên dịch vụ chứa ký tự ',' không cho phép. Xin thay thế = '.'", "Chú ý"); 
                return;
            }
            try
            {
        ...
        void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridView1.Columns[ColumnUpdate.Name].Index)
            {
                ...
                if (name.Contains(','))
                {
                    MessageBox.Show("Tên dịch vụ chứa ký tự ',' không cho phép. Đã thay thế = '.'", "Chú ý");
                    dataGridView1.Rows[e.RowIndex].Cells[this.ColumnName.Name].Value = name.Replace(',', '.');
                    name = dataGridView1.Rows[e.RowIndex].Cells[this.ColumnName.Name].Value.ToString();
                }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string strCommand = "Select * From history";
            ...
            using (DbDataReader reader = db.ExecuteReader(strCommand, null) as DbDataReader)
            {
                while (reader.Read())
                {
                    try
                    {
                        string Medicine = reader[ClinicConstant.HistoryTable_Medicines].ToString();
                        string Idhistory = reader[ClinicConstant.HistoryTable_IdHistory].ToString();
                        if (Medicine.Contains(textBox1.Text))
                        {
                            Medicine = Medicine.Replace(textBox1.Text,textBox1.Text.Replace(',','.'));

                            string strCommandUpdate = "Update history Set Medicines = " + Helper.ConvertToSqlString(Medicine) + " Where IdHistory = " + Idhistory;

                            DatabaseFactory.Instance2.ExecuteNonQuery(strCommandUpdate, null);
